...
ipcMain.on(
  "write-to-file",
  (event: any, fileName: string, message: string): void => {
    fs.writeFile(fileName, message, (err): void => {
      if (err) {
        console.log("Cant`t write to file", fileName);
      }
    });
    fs.readFile(fileName, "utf8", (err: any, message: string) => {
      if (err) {
        return console.log(err);
      }
      event.sender.send("get-message", message);
      windows.forEach((currentWindow: BrowserWindow, index: number) => {
        if (currentWindow) {
          currentWindow.webContents.send("get-message", `${message}: ${index}`);
        }
      });
    });
  }
...
ipcMain.handle("get-counter", (event, key) => {
  return store.get(key);
});

ipcMain.on("change-store", (event, key, value) => {
  store.set(key, value);
});

store.onDidChange("counter", (newValue, oldValue) => {
  console.log(oldValue, newValue);
  mainWindow?.webContents.send("get-new-counter", store.get("counter"));
  windows.forEach((currentWindow: BrowserWindow) => {
    if (currentWindow) {
      currentWindow.webContents.send("get-new-counter", store.get("counter"));
    }
  });
});
